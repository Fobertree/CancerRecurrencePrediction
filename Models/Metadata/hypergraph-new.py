...
from torch_geometric.nn import HypergraphConv, global_mean_pool, TopKPooling
...
# DATA PREPROCESSING
# -------------------------------
METADATA_PATH = "/Users/alexanderliu/EmoryCS/CancerRecurrencePrediction/Data/new_metadata.csv"
...
class HypergraphNN(nn.Module):
    def __init__(self, in_channels, hidden_channels=128, dropout=0.2, use_topk=False, topk_ratio=0.8):
        ...
        # Optional Top-K pooling
        if use_topk:
            self.pool = TopKPooling(hidden_channels, ratio=topk_ratio)
    ...
    def forward(self, x, edge_index):
        ...
        # Optional Top-K pooling (focus on important nodes)
        batch_vec = torch.zeros(x.size(0), dtype=torch.long, device=x.device)  # all nodes belong to single graph
        if self.use_topk:
            x, edge_index, _, batch_vec, _, _ = self.pool(x, edge_index, batch_vec)

        # Global mean pooling to get graph-level embedding
        # x_graph = global_mean_pool(x, batch_vec)  # shape [1, hidden_channels]

        # Final binary classification logits
...
if False:
    ...
    with torch.no_grad():
        for batch_X, batch_y in test_dataloader:
            preds = model(batch_X.to(device), test_edge_index.to(device)).cpu().numpy().flatten()
            test_preds.append(preds)

    test_preds = torch.cat(test_preds, dim=-1).flatten()
